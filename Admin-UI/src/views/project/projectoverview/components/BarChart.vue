  ...
  methods: {
    getCountTaskUserByProjectId(id) {
      ...
      getCountTaskUserByProjectId(id).then(response => {
        this.userTaskCount = response.data
        response.data.forEach((item, index) => {
          this.userList.push(item.name)
          this.taskCount.push(item.value)
        })
      })
    },
    initChart() {
      ...
      this.chart.setOption({
        ...
        xAxis: [{
          type: 'category',
          data: this.userList, // ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
          axisTick: {
            alignWithLabel: true
          }
        }],
        ...
        series: [{
          ...
          stack: 'vistors',
          barWidth: '25%',
          data: this.taskCount,  // [79, 52, 200, 334, 390, 330, 220],
          animationDuration
        }]
      })
    }
